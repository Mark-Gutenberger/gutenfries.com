...
pub const COLOR_SHADES_ARRAY: [&str; 10] =
	["50", "100", "200", "300", "400", "500", "600", "700", "800", "900"];

pub const COLOR_SHADES_ARRAY_LIGHT: [&str; 4] = ["200", "300", "400", "500"];
...
pub mod methods {
	...
	/// converts a tailwind v2 css color into a hex value
	/// ## Example:
	/// ```
	/// let hex = color_to_hex("blue-600");
	/// ```
	/// output: `#2563eb`
	pub fn color_to_hex(tw_color: &str) -> String {
		let tw_color = tw_color.split('-').collect::<Vec<&str>>();
		let color = tw_color[0];
		let shade = tw_color[1];
		let mut hex = "";

		// find the color
		for (name, shades) in super::colors.iter() {
			if name == &color {
				// find the shade
				for shade_pair in shades.iter() {
					if shade_pair[0] == shade {
						hex = shade_pair[1];
					}
				}
			}
